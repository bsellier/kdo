...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create giftss-ccce
  fly apps create giftss-ccce-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app giftss-ccce
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app giftss-ccce-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'giftss-ccce-staging' does not match app name in config file 'giftss-ccce'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name giftss-ccce-db
  fly postgres attach --app giftss-ccce giftss-ccce-db

  fly postgres create --name giftss-ccce-staging-db
  fly postgres attach --app giftss-ccce-staging giftss-ccce-staging-db
  ```
